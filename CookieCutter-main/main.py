...
# Set up folder path and load images
folderPath = 'frames'
...
while not gameOver:
    ...
    time =0
    h,w,_ = sqr_img.shape
    img = overlayPNG(img,sqr_img,[cordinatex,cordinatey]) # Place the cookie image
    img = overlayPNG(img,mlsa,[0,0]) # Place the mlsa image
            ...
            if cordinatex < smoothX < cordinatex + w and cordinatey < smoothY < cordinatey + h:
                ...
                if colorblue == b and colorgreen == g and colorred == r:
                    ...
                    if px!=cursor[0] and py!=cursor[1]:
                        ...
                        if(smoothX-10 <= fx <= smoothX
 + 10 and smoothY-10 <= fy <= smoothY):
                            if corners == [1,1,1,1]:
                                print("YOU WON!!")
                                NotWon = False
                                gameOver =True
